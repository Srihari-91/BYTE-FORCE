...
def useful_vs_noise_from_tfidf(
    words: list[str],
    word_scores: dict[str, float],
) -> tuple[int, int]:
    """
    Count word *occurrences* classified as useful vs noise using in-document TF-IDF.
    Low TF-IDF (bottom quartile of scores) marks common filler; known filler lemmas
    always count as noise. Degenerate TF-IDF falls back to stopword/filler heuristics.
    """
    import math
    ...
    if not words:
        return 0, 0
    n = len(words)
    vals = [float(word_scores.get(w, 0.0)) for w in words]
    mx = max(vals) if vals else 0.0

    if mx <= 0.0:
        noise = sum(1 for w in words if w in ENGLISH_STOP_WORDS or w in NOISE_FILLERS)
    ...
    if n >= 4:
        ...
        noise_idx.update(order[:k])
    for i, w in enumerate(words):
        if w in NOISE_FILLERS:
            noise_idx.add(i)
...
def noise_word_set_from_tfidf(words: list[str], word_scores: dict[str, float]) -> set[str]:
    """Lexemes treated as low-signal noise for diff highlighting (union-friendly)."""
    import math
    ...
    if mx <= 0.0:
        for w in words:
            if w in ENGLISH_STOP_WORDS or w in NOISE_FILLERS:
                out.add(w)
        return out
    if n >= 4:
        k = max(1, math.ceil(n * 0.25))
